    ...
    <div className="bg-body min-h-screen dark:bg-text">
      ...
      <section id="profile-container">
        <div className="container-1 pt-20">
          <div className="my-10">
            <h1 className="font-semibold text-3xl mb-3 text-text dark:text-white">My Profile</h1>
            <p className="text-text/70 font-semibold dark:text-white">Manage your account information and preferences</p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 mb-15 md:grid-cols-12 items-start">
            ...
            <div className="md:col-span-5 bg-primary p-6 rounded-xl shadow-sm border  dark:bg-[#121212]">
              ...
              <div className="flex gap-5 border-b-2 pb-5 items-center">
                ...
                <div>
                  <h1 className="font-semibold text-text text-xl text text dark:text-white">{user.first_name} {user.last_name}</h1>
                  <p className="text-text/50 text-sm text text dark:text-white">{user.email}</p>
                </div>
              </div>
  ...
  <div className="relative">
    ...
    <div className="bg-primary dark:bg-[#121212] py-10 h-80 md:h-100 transition-colors duration-300">
      <div className="md:max-w-7xl md:mx-auto">
            <h1 className=" mx-3 text-center font-semibold text-xl  mb-5 md:text-start dark:text-white">Registered Vehicle:</h1> 
    ...
    <thead>
      <tr className="text-text/50 font-bold uppercase text-xs tracking-wider dark:text-white">
        <th className="px-4 py-3 border-b border-gray-100">Plate No.</th>
        <th className="px-4 py-3 border-b border-gray-100">Wheel Type:</th>
    ...
    <tbody className="text-text font-semibold text-sm">
      <tr className="hover:bg-dark/10 rounded-4xl transition-colors duration-200 dark:text-white">
        ...
        <td className="px-4 py-4 text-right">
          <button onClick={() => setVehicleModalOpen(true)}  className="bg-gray-200 text-text px-6 py-1.5 rounded-full font-bold hover:bg-red-500 hover:text-white transition-all duration-300 text-xs hover:cursor-pointer">
            Edit
          </button>
        </td>
      </tr>
...
<Dialog open={isProfileModalOpen} onOpenChange={setProfileModalOpen}>
  <DialogContent className="bg-primary rounded-3xl p-8 border-none">
    <DialogHeader>
      <DialogTitle className="text-2xl font-bold text-text">Edit Information</DialogTitle>
    </DialogHeader>
...
<Dialog open={isVehicleModalOpen} onOpenChange={setVehicleModalOpen} >
  <DialogContent className="max-w-sm rounded-3xl px-10 border-none bg-primary">
      <DialogHeader>
      <DialogTitle className="text-xl font-bold text-text">Update Vehicle</DialogTitle>
    </DialogHeader>
      ...
      <form onSubmit={handleUpdateChanges} className="space-y-4 py-4">
        <div className="space-y-1">
          ...
          onChange={(e) => setUserVehicle({...userVehicle, plate_no: e.target.value.toUpperCase()})}
          className="uppercase"
        />
      </div>
        ...
        <div className="space-y-1 mt-5">
          ...
          onValueChange={(value) => setUserVehicle({...userVehicle, wheel_type: value})}
        >
          <SelectTrigger>
       ...
       <div className="flex gap-3 pt-4">
        <DialogClose asChild>
          <Button type="button" variant="secondary" className="flex-1 rounded-xl bg-text/10 hover:bg-secondary duration-300 ease-in-out font-semibold text-sm hover:text-primary">
            Cancel
          </Button>
        </DialogClose>
        <Button 
